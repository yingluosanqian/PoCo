...
- 当前 workdir 已开始进入文本和 card 两条 task 创建路径，但 approval/result 仍未形成完整 card-first 闭环
...
- 当前 DM 已能主动下发首页卡片，并具备第一批真实 callback 动作；但 project 命名、approval/result 卡片、session/task 正式交互仍未形成完整正式工作流
- 当前正式实现仍是 DM 首屏卡片 + group card/task 混合态，尚未形成完整 card-first 正式交互面
